...
function configure(config) {
  ...
  if (is.string(config.partialsdir)) {
    CONFIG.partialsdir = config.partialsdir;
  }
  ...
  if (is.array(config.arguments)) {
    CONFIG.arguments = CONFIG.arguments.concat(config.arguments);
  }
...
function load(config) {
  return new Promise((resolve) => {
    ...
    walker.on('file', (root, stats, next) => {
      const isPartial = path.normalize(path.resolve(root))
        .indexOf(path.normalize(path.resolve(CONFIG.partialsdir))) !== -1;
...
function partial(name) {
  ...
  if (is.not.string(CONFIG.partialsdir)) {
  ...
  const partial = path.normalize(path.resolve(
    path.join(CONFIG.partialsdir, dirname, basename + '.js')
  ));
